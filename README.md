...
- 配信中の確認済みrevision: `yorisoi-alagille-00019-bkm`
...
- 初回登録は、メールアドレス・パスワードと最小限の呼び名だけを2段階で登録
- 自分の記録では呼び名1件、家族の記録では利用者と記録する方の呼び名2件だけで開始
- 生年月日とアバターは、初回ホームまたはメニューから後で設定
...
開発者・関係者だけが使う非公開の固定デモアカウントを用意できます。通常利用者と同じFirebase Authentication、HttpOnlyセッション、家族・記録対象者の認可経路を通り、ホームでは「デモデータ」と表示します。

認証情報はGit対象外の`secure/developer-demo-account.env`へ置きます。ファイルには次の2項目だけを記載します。

```text
YORISOI_DEMO_EMAIL=...
YORISOI_DEMO_PASSWORD=...
```

設定内容を確認します。この時点では外部データを変更しません。

```powershell
.\scripts\reset-developer-demo-account.ps1
```

固定したデモ家族だけを架空データの初期状態へ戻します。

```powershell
.\scripts\reset-developer-demo-account.ps1 -Apply
```

このスクリプトは、デモ用Firebaseユーザー、専用家族、記録する方、検査、お薬、成長、日々の様子、受診予定、質問、診察メモを作成または再投入します。実在する患者情報は使用しません。
