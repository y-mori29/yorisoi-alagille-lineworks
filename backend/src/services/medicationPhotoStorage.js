...
async function saveMedicationPhoto({ tenantId, familyId, patientId, photoDataUrl }) {
    ...
    const objectName = [
        ...
        'families', familyId,
        'patients', patientId,
        'medications', `${randomUUID()}.${extensions[mimeType] || 'jpg'}`,
    ].join('/');
...
async function deleteMedicationPhoto(objectName) {
    if (!objectName || !process.env.GCS_BUCKET) return;
    await bucket.file(objectName).delete({ ignoreNotFound: true });
}

function createMedicationPhotoReadStream(objectName) {
    assertBucketConfigured();
    return bucket.file(objectName).createReadStream();
}

module.exports = { saveMedicationPhoto, deleteMedicationPhoto, createMedicationPhotoReadStream };
